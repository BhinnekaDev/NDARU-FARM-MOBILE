...
const MyButtonQuantityProduct: React.FC<MyButtonQuantityProductProps> = ({ selectedQuantity: externalSelectedQuantity, onSelectQuantity, quantityType = "kilogram" }) => {
  const colorScheme = useColorScheme();
  const isDark = colorScheme === "dark";
  const scrollViewRef1 = useRef<ScrollView>(null);
  const scrollViewRef2 = useRef<ScrollView>(null);

  const [selectedQuantity, setSelectedQuantity] = useState<number>(externalSelectedQuantity || 1);
  ...
  const quantities = quantityType === "bulan" ? Array.from({ length: 12 }, (_, i) => i + 1) : Array.from({ length: 10 }, (_, i) => i + 1);
  ...
  return (
    <View className="flex-col w-full gap-4 py-2">
      <ScrollView ref={scrollViewRef1} horizontal={true} showsHorizontalScrollIndicator={false}>
        <View className="flex-row gap-2">
          {quantities.slice(0, quantityType === "bulan" ? 6 : 5).map((qty, index) => {
            ...
            return (
              <TouchableOpacity
                ...
                style={{
                  backgroundColor: isSelected ? (isDark ? "#333836" : "#159778") : "transparent",
                  borderColor: isSelected ? (isDark ? "#333836" : "#159778") : isDark ? "white" : "black",
                }}
              >
                <Text
                  className="text-sm"
      ...
      <ScrollView ref={scrollViewRef2} horizontal={true} showsHorizontalScrollIndicator={false}>
        <View className="flex-row gap-2">
          {quantities.slice(quantityType === "bulan" ? 6 : 5).map((qty, index) => {
            const isSelected = selectedQuantity === qty;
            return (
              <TouchableOpacity
                key={qty}
                onPress={() => handleSelectQuantity(qty, index + (quantityType === "bulan" ? 6 : 5))}
                className="border py-1.5 px-5 rounded-2xl"
                style={{
                  backgroundColor: isSelected ? (isDark ? "#333836" : "#159778") : "transparent",
                  borderColor: isSelected ? (isDark ? "#333836" : "#159778") : isDark ? "white" : "black",
